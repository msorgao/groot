...
from gr00t.experiment.trainer import Gr00tTrainer, ProfCallback, OCMSSummaryCallback
...
def run(config: Config):
    ...
    # Initialize OCMS Summary for loss reporting (always on main process)
    if global_rank == 0:
        global _summary_instance
        _summary_instance = Summary()
        _summary_instance.init(name='loss', x='step', ys=['loss'], series=[], description='Training Loss', smooth=False)
        logging.info("OCMS Summary initialized for loss reporting")
    ...
    # Add OCMS Summary callback if available
    if global_rank == 0 and _summary_instance is not None:
        trainer.add_callback(OCMSSummaryCallback(_summary_instance))
        logging.info("Added OCMS Summary callback to trainer")
    ...
    # Report final loss to OCMS if available
    if global_rank == 0 and _summary_instance is not None:
        final_step = trainer.state.global_step
        final_loss = trainer.loss.item() if trainer.loss is not None else 0.0
        _summary_instance.loss.add(step=final_step, values=[final_loss])
        logging.info(f"Reported final loss to OCMS: step={final_step}, loss={final_loss}")
