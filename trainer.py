...
class OCMSSummaryCallback(TrainerCallback):
    """Callback to report training loss to OCMS Summary periodically."""
    
    def __init__(self, summary_instance):
        self.summary_instance = summary_instance
        self.logger = logging.getLogger(__name__)
    
    def on_log(self, args, state, control, logs=None, **kwargs):
        """Called when logging occurs, report loss to OCMS."""
        if self.summary_instance is None:
            return
        
        # 确保只在主进程上报（避免 DDP 下重复上报）
        ...
        if logs is not None:
            ...
            elif hasattr(state, "global_step"):
                # Fallback to trainer's stored loss
                trainer = kwargs.get("trainer")
                if trainer is not None and hasattr(trainer, "loss") and trainer.loss is not None:
                    loss_value = trainer.loss.item()
        
        # Report to OCMS
        if loss_value is not None:
            global_step = state.global_step
            self.summary_instance.loss.add(step=global_step, values=[loss_value])
            self.logger.info(f"Reported loss to OCMS: step={global_step}, loss={loss_value}")
